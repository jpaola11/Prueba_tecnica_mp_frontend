...
export const caseFileService = {
  async list(): Promise<CaseFileListResponse> {
    const { data } = await httpClient.get<CaseFileListResponse>("/api/case-file");
    return data;
  },

  async findOne(id: number): Promise<CaseFileDto> {
    const { data } = await httpClient.get<CaseFileDto>(`/api/case-file/${id}`);
    return data;
  },
  ...
  async remove(id: number): Promise<void> {
    await httpClient.delete(`/api/case-file/${id}`);
  },
};
